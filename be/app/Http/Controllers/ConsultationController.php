...
class ConsultationController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $consultation->reason = $validated['reason'] ?? $consultation->reason;
            $consultation->instruction = $validated['instruction'] ?? $consultation->instruction;
            $consultation->examination = $validated['examination'] ?? $consultation->examination;
            $consultation->diagnosis = $validated['diagnosis'] ?? $consultation->diagnosis;
            $consultation->management = $validated['management'] ?? $consultation->management;
            $consultation->investigation = $validated['investigation'] ?? $consultation->investigation;

            // Update status if diagnosis is provided
            if (!empty($validated['diagnosis'])) {
                $consultation->status = 4; // Status 4 = Notes Added/Completed
            }

            $consultation->save();

            // Load relationships for response
            $consultation->load('doctor', 'location', 'creator', 'patient', 'prescription.items.drug', 'medicalHistory');
    ...
    public function byPatient($patientId, Request $request)
    {
        $consultations = Consultation::with([
            ...
            'creator', 
            'patient', 
            'prescription.items.drug'  // Load prescription with items and drug details
        ])
            ...
            ->map(function ($c) {
                ...
                if ($c->prescription) {
                    $prescriptionData = [
                        ...
                        'created_at' => $c->prescription->created_at,
                        'items' => $c->prescription->items->map(function($item) {
                            return [
                                'id' => $item->id,
